...
def create_baseline_config():
    ...
    # 读取对比配置文件
    with open(ROOT / "data/hyps/hyp.baseline_vs_focal.yaml", 'r') as f:
...
def compare_results():
    ...
    if baseline_results.exists() and focal_results.exists():
        ...
        # 这里可以添加更详细的结果分析代码
        print("\n💡 请查看以下目录的详细结果:")
        print(f"   - 基线实验: runs/train/baseline_bce_focal/")
        print(f"   - Focal Loss实验: runs/train/focal_loss_gamma2/")
...
def main():
    ...
    parser.add_argument("--mode", choices=["baseline", "focal", "compare", "all"], 
