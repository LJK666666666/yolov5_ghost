...
"""
Run YOLOv5 detection inference on videos and streams.

Usage:
    $ python tools/video.py --weights models_trained/Ghost_e10_0626/weights/best.pt --source 0  # webcam
                                                                                    vid.mp4  # video
"""
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if view_img:
                ...
                if cv2.waitKey(1) == ord("q"):  # 1 millisecond, 'q' to quit
                    break
        if not webcam:
...
def parse_opt():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default=ROOT / "models_trained/Ghost_e10_0626/weights/best.pt",
        help="model path or triton URL",
    )
    parser.add_argument(
        "--source", type=str, default="0", help="file/dir/URL/glob/screen/0(webcam)"
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        "--view-img", action="store_true", default=True, help="show results"
    )
