#!/usr/bin/env python3
"""增强VAE的生成脚本 适配EnhancedPersonCentricVAE模型."""
...
import numpy as np
import torch
from enhanced_person_vae import LATENT_DIMS, ORIGINAL_SIZE, EnhancedPersonCentricVAE, composite_scene_enhanced
...
class EnhancedGenerator:
    ...
    def generate_controlled(self, controls=None, num_samples=1):
        ...
        with torch.no_grad():
            ...
            decoded = {"background": background, "person": person_decoded}
        ...
        with torch.no_grad():
            for i in range(steps):
                ...
                final_image = composite_scene_enhanced(
                    decoded["background"], decoded["person"], target_size=ORIGINAL_SIZE, feather=25
                )
...
def create_quality_comparison(model_path, output_dir):
    ...
    for feather in feather_values:
        final_image = composite_scene_enhanced(
            decoded["background"], decoded["person"], target_size=ORIGINAL_SIZE, feather=feather
        )
