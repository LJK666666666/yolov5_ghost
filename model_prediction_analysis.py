    ...
    for pred in predictions:
        ...
        if best_gt_idx >= 0:
            matched_predictions.append({
                'prediction': pred,
                'ground_truth': unmatched_gt[best_gt_idx],
                'iou': best_iou
            })
...
def draw_predictions_and_ground_truth(image, predictions, ground_truth, class_names):
    ...
    for gt in ground_truth:
        ...
        cv2.putText(img_copy, f"GT: {class_names[gt['class_id']]}", 
    ...
    for pred in predictions:
        ...
        cv2.putText(img_copy, f"Pred: {class_names[pred['class_id']]} ({pred['confidence']:.2f})", 
        ...
        if has_errors:
            ...
            if total_fp > 0:
                ...
                error_info = {
                    ...
                    'class_breakdown': {class_names[i]: stats['false_positives']
                                      for i, stats in class_stats.items() if stats['false_positives'] > 0}
                }
            ...
            if total_fn > 0:
                ...
                error_info = {
                    ...
                    'total_predictions': len(predictions),
                    'total_ground_truth': len(ground_truth),
                    'class_breakdown': {class_names[i]: stats['false_negatives']
                                      for i, stats in class_stats.items() if stats['false_negatives'] > 0}
                }
            ...
            if total_low_iou > 0:
                ...
                error_info = {
                    ...
                    'total_predictions': len(predictions),
                    'total_ground_truth': len(ground_truth),
                    'class_breakdown': {class_names[i]: stats['low_iou_matches']
                                      for i, stats in class_stats.items() if stats['low_iou_matches'] > 0}
                }
            ...
            if total_conf_issues > 0:
                ...
                error_info = {
                    ...
                    'total_predictions': len(predictions),
                    'total_ground_truth': len(ground_truth),
                    'class_breakdown': {class_names[i]: stats['confidence_issues']
                                      for i, stats in class_stats.items() if stats['confidence_issues'] > 0}
                }
...
def main():
    ...
    parser.add_argument('--weights', default='runs/train200to300epoch/yolov5s_/weights/best.pt', 
                       help='模型权重路径')
