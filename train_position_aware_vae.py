...
from clustered_template_vae import TEMPLATE_SIZE, LATENT_DIMS, NUM_CLUSTERS
...
class PositionAwareVAE(nn.Module):
    ...
    def __init__(self, clustered_templates, cluster_models):
        ...
        for category in self.categories:
            ...
            # 外观编码器/解码器
            self.appearance_encoders[category] = AppearanceEncoder(
                category, template_size, latent_dim, num_clusters
            )
            self.appearance_decoders[category] = AppearanceDecoder(
                category, template_size, latent_dim
            )
...
class AppearanceDecoder(nn.Module):
    ...
    def forward(self, z, template):
        ...
        if difference.shape != template.shape:
            difference = F.interpolate(difference, size=template.shape[2:],
...
class PositionAwareDataset(Dataset):
    ...
    def _extract_corrected_sample(self, image, annotations):
        ...
        if np.sum(all_objects_mask) / (h * w) < 0.5:
            ...
            mask_resized = cv2.resize(background_region_mask.astype(np.float32),
                                    TEMPLATE_SIZE['background'])
            sample['background_mask'] = mask_resized

            # 添加统计信息用于调试
            pure_bg_ratio = np.sum(background_region_mask) / (h * w)
            sample['background_stats'] = {
                'pure_bg_ratio': pure_bg_ratio,
                'person_region_ratio': 1 - pure_bg_ratio
            }
...
def compute_position_aware_loss(outputs, targets, differences, latent_params,
                              cluster_logits, position_params,
                              background_masks=None, position_targets=None,
                              true_clusters=None, beta=1.0, gamma=1.0, lambda_pos=0.5,
                ...
                if position_targets and category in position_targets:
                    ...
                    if pred_batch_size != target_batch_size:
                        ...
                        if min_batch_size > 0:
                            ...
                            position_kl_loss = -0.5 * torch.sum(
                                1 + pred_pos_logvar - pred_pos_mu.pow(2) - pred_pos_logvar.exp()
                            ) / pred_pos_mu.size(0)
...
def train_position_aware_vae(data_dir, output_dir, epochs=50, batch_size=4, lr=1e-4,
                           max_samples=None, person_region_weight=0.3):
    """训练位置感知的VAE"""
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            loss, loss_components = compute_position_aware_loss(
                outputs, batch, differences, latent_params, cluster_logits,
                position_params, background_masks, position_targets,
                beta=beta, gamma=gamma, lambda_pos=lambda_pos,
...
def main():
    ...
    parser.add_argument('--person_weight', type=float, default=0.3,
                       help='人物区域在背景损失中的权重 (0.0-1.0)')

    args = parser.parse_args()

    train_position_aware_vae(
        args.data, args.output, args.epochs,
        args.batch_size, args.lr, args.max_samples, args.person_weight
    )
