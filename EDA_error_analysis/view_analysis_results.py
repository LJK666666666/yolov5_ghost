...
def view_analysis_results(analysis_dir="test_prediction_analysis"):
    ...
    if os.path.exists(overall_stats_file):
        with open(overall_stats_file) as f:
    ...
    if overall_stats:
        ...
        for error_type, count in sorted_errors[:3]:  # 显示前3个主要问题
            if count > 0:
                ...
                suggestions = {
                    "false_positives": ["提高置信度阈值", "使用更严格的NMS参数", "增加负样本训练", "使用Focal Loss"],
                    "false_negatives": ["降低置信度阈值", "增加数据增强", "使用多尺度训练", "调整anchor尺寸"],
                    "low_iou_matches": [
                    ...
                    "confidence_issues": ["使用置信度校准技术", "调整分类损失权重", "使用标签平滑", "增加困难样本挖掘"],
                }
