...
"""
层次化安全装备检测分析脚本
专门处理person、helmet、safety_vest之间的层次关系
分析人员安全装备穿戴情况
"""
...
def hierarchical_match_analysis(predictions, ground_truth, class_names, iou_threshold=0.5, overlap_threshold=0.3):
    ...
    for pred_person in pred_persons:
        ...
        if best_gt_idx >= 0:
            ...
            # 分析预测person的装备
            pred_equipment = analyze_person_equipment(
                pred_person['bbox'], pred_helmets, pred_vests, overlap_threshold
            )
            
            # 分析真实person的装备
            gt_equipment = analyze_person_equipment(
                matched_gt['bbox'], gt_helmets, gt_vests, overlap_threshold
            )
            ...
            unmatched_pred_persons.append({
                'person': pred_person,
                'equipment': pred_equipment
            })
    ...
    for gt_person in unmatched_gt_persons:
        gt_equipment = analyze_person_equipment(
            gt_person['bbox'], gt_helmets, gt_vests, overlap_threshold
        )
        unmatched_gt_persons_with_equipment.append({
            'person': gt_person,
            'equipment': gt_equipment
        })
...
def draw_hierarchical_analysis(image, analysis_result, class_names):
    ...
    for unmatched in analysis_result['unmatched_pred_persons']:
        ...
        cv2.putText(img_copy, f"FP Person: {status}", (x1, y1-10),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors['person_fp'], 2)
    ...
    for unmatched in analysis_result['unmatched_gt_persons']:
        ...
        cv2.putText(img_copy, f"FN Person: {status}", (x1, y1-10),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors['person_fn'], 2)
    ...
    for helmet in analysis_result['independent_pred_helmets']:
        ...
        cv2.putText(img_copy, "Independent Helmet", (x1, y1-10),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, colors['helmet'], 2)
    ...
    for vest in analysis_result['independent_pred_vests']:
        ...
        cv2.putText(img_copy, "Independent Vest", (x1, y1-10),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, colors['safety_vest'], 2)

    return img_copy

def process_dataset_hierarchical(model, images_path, labels_path, output_dir, split_name, class_names, conf_threshold=0.25):
    """处理数据集的层次化分析"""
    print(f"\n处理{split_name}...")
    ...
    for i, img_file in enumerate(image_files):
        ...
        for status in overall_stats['equipment_analysis']:
            for result_type in overall_stats['equipment_analysis'][status]:
                overall_stats['equipment_analysis'][status][result_type] += \
                    img_stats['equipment_analysis'][status][result_type]

        for component in overall_stats['component_detection']:
            for metric in overall_stats['component_detection'][component]:
                overall_stats['component_detection'][component][metric] += \
                    img_stats['component_detection'][component][metric]

        # 判断是否有错误 - 使用更合理的标准
        # 主要关注Person检测错误，装备状态错误相对宽松
        has_person_errors = (img_stats['person_detection']['false_positives'] > 0 or
                           img_stats['person_detection']['false_negatives'] > 0)

        has_equipment_errors = any(
            img_stats['equipment_analysis'][status]['incorrect'] > 0
            for status in img_stats['equipment_analysis']
        )

        has_component_errors = (
            len(analysis_result['independent_pred_helmets']) > 0 or
            len(analysis_result['independent_pred_vests']) > 0 or
            len(analysis_result['independent_gt_helmets']) > 0 or
            len(analysis_result['independent_gt_vests']) > 0
        )

        # 修改错误判断逻辑：主要基于Person检测，装备状态错误不算致命错误
        has_errors = has_person_errors  # 只有Person检测错误才算真正的错误
        has_minor_errors = has_equipment_errors or has_component_errors  # 次要错误
    ...
    # 计算性能指标
    person_precision = overall_stats['person_detection']['true_positives'] / (
        overall_stats['person_detection']['true_positives'] + overall_stats['person_detection']['false_positives']
    ) if (overall_stats['person_detection']['true_positives'] + overall_stats['person_detection']['false_positives']) > 0 else 0

    person_recall = overall_stats['person_detection']['true_positives'] / (
        overall_stats['person_detection']['true_positives'] + overall_stats['person_detection']['false_negatives']
    ) if (overall_stats['person_detection']['true_positives'] + overall_stats['person_detection']['false_negatives']) > 0 else 0

    person_f1 = 2 * (person_precision * person_recall) / (person_precision + person_recall) \
        if (person_precision + person_recall) > 0 else 0

    # 计算装备状态准确率
    equipment_accuracy = {}
    for status in overall_stats['equipment_analysis']:
        total = (overall_stats['equipment_analysis'][status]['correct'] +
    ...
    no_helmet_gt = (overall_stats['equipment_analysis']['vest_only']['correct'] +
    ...
    no_helmet_correct = (overall_stats['equipment_analysis']['vest_only']['correct'] +
                        overall_stats['equipment_analysis']['no_equipment']['correct'])
    ...
    no_vest_gt = (overall_stats['equipment_analysis']['helmet_only']['correct'] +
                 overall_stats['equipment_analysis']['helmet_only']['incorrect'] +
                 overall_stats['equipment_analysis']['no_equipment']['correct'] +
                 overall_stats['equipment_analysis']['no_equipment']['incorrect'])

    no_vest_correct = (overall_stats['equipment_analysis']['helmet_only']['correct'] +
                      overall_stats['equipment_analysis']['no_equipment']['correct'])
...
def main():
    ...
    parser.add_argument('--weights', default='runs/rail_train300epoch/yolov5s_/weights/best.pt',
                       help='模型权重路径')
    ...
    for split_name, (img_dir, label_dir) in splits.items():
        ...
        if os.path.exists(images_path) and os.path.exists(labels_path):
            stats, _ = process_dataset_hierarchical(
                model, images_path, labels_path, args.output,
                split_name, class_names, args.conf
            )
