...
def main():
    ...
    # 分析标注
    print(f"\n4. 标注分析:")
    train_annotations, train_class_counts, train_bbox_areas, train_bbox_widths, train_bbox_heights, train_objects_per_image = parse_yolo_labels(train_labels_path)
    ...
    # 总结和建议
    print(f"\n8. 总结和建议:")
    print(f"   ✓ 数据集规模适中，适合训练YOLO模型")
    
    if len(train_class_counts) == 2:
        class_balance = abs(list(train_class_counts.values())[0] - list(train_class_counts.values())[1]) / total_annotations
        if class_balance < 0.3:
            print(f"   ✓ 类别分布相对均衡")
        else:
            print(f"   ⚠ 类别分布不均衡，建议使用数据增强或权重平衡")
    
    if train_widths and (max(train_widths) - min(train_widths)) > 500:
        print(f"   ⚠ 图像尺寸变化较大，建议使用多尺度训练")
    else:
        print(f"   ✓ 图像尺寸相对一致")
    
    print(f"   ✓ 建议使用数据增强技术提高模型泛化能力")
    print(f"   ✓ 可以考虑使用预训练模型进行迁移学习")
