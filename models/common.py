...
class CoordinateAttention(nn.Module):
    """Coordinate Attention for efficient attention mechanism that preserves positional information."""

    def __init__(self, c1, c2, reduction=32):
        """Initializes CoordinateAttention with input channels, output channels and reduction ratio."""
        super().__init__()
        # Remove AdaptiveAvgPool2d to avoid deterministic algorithm issues
    ...
    def forward(self, x):
        """Forward pass with coordinate attention mechanism."""
        identity = x

        n, c, h, w = x.size()

        # Use deterministic-compatible pooling operations
        # H pooling: average over width dimension
        x_h = torch.mean(x, dim=3, keepdim=True)  # [N, C, H, 1]
        # W pooling: average over height dimension
