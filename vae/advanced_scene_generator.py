...
class AdvancedSceneGenerator:
    ...
    def generate_controlled_scene(
        ...
        with torch.no_grad():
            ...
            # 4. 合成场景
            final_scene = self._compose_scene_advanced(background, person_data, helmet_data, vest_data, scene_size)
    ...
    def _generate_background(self, cluster_id=None):
        ...
        return {"image": output[0], "cluster_id": cluster_id, "latent": z[0]}
    ...
    def _generate_person(self, cluster_id=None, position=None, scene_size=(360, 640)):
        ...
        return {"image": output[0], "cluster_id": cluster_id, "latent": z[0], "position": position}
    ...
    def _compose_scene_advanced(self, background, person_data, helmet_data, vest_data, scene_size):
        ...
        bg_resized = F.interpolate(
            background["image"].unsqueeze(0), size=(scene_h, scene_w), mode="bilinear", align_corners=True
        )[0]
        scene = bg_resized
        ...
        if person_w > 0 and person_h > 0:
            person_resized = F.interpolate(
                person_data["image"].unsqueeze(0), size=(person_h, person_w), mode="bilinear", align_corners=True
            )[0]
        ...
        if helmet_w > 0 and helmet_h > 0:
            helmet_resized = F.interpolate(
                helmet_data["image"].unsqueeze(0), size=(helmet_h, helmet_w), mode="bilinear", align_corners=True
            )[0]
        ...
        if vest_w > 0 and vest_h > 0:
            vest_resized = F.interpolate(
                vest_data["image"].unsqueeze(0), size=(vest_h, vest_w), mode="bilinear", align_corners=True
            )[0]
...
def generate_diverse_scenes(model_path, output_dir, num_samples=20):
    ...
    for i in tqdm(range(num_samples), desc="随机场景"):
        ...
        cv2.imwrite(str(output_dir / f"random_scene_{i:03d}.jpg"), cv2.cvtColor(scene_img, cv2.COLOR_RGB2BGR))
    ...
    for bg_id in bg_clusters[:3]:  # 前3个背景聚类
        for person_id in person_clusters[:3]:  # 前3个人物聚类
            scene_data = generator.generate_controlled_scene(bg_cluster=bg_id, person_cluster=person_id)
