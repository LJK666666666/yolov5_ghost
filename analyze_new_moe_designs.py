...
def analyze_moe_config(yaml_path: str, model_name: str) -> Dict[str, Any]:
    ...
    for i, layer_config in enumerate(all_layers):
        ...
        if module in ['C3MoE', 'C3HybridMoE']:
            ...
            if module == 'C3MoE' and len(args) >= 6:
                ...
                analysis['moe_layers'].append(moe_info)
                print(f"  {i:2d}. {module:12s} - {num_experts}专家激活{top_k}个+共享({shared_ratio:.1%}) = 总激活{total_activation_ratio:.1%}")
        else:
    ...
    if analysis['moe_layers']:
        if 'total_activation_ratio' in analysis['moe_layers'][0]:
            # 混合MoE
            avg_activation = sum(layer['total_activation_ratio'] for layer in analysis['moe_layers']) / len(analysis['moe_layers'])
        else:
            # 标准MoE
            avg_activation = sum(layer['activation_ratio'] for layer in analysis['moe_layers']) / len(analysis['moe_layers'])
...
def compare_designs():
    ...
    for item in comparison_table:
        print(f"{item['model']:<25} {item['moe_layers']:<8} {item['avg_moe_activation']:<12.1f}% {item['overall_activation']:<10.1f}%")
    ...
    if len(comparison_table) >= 2:
        ...
        if hybrid:
            ...
            print(f"  通过共享专家保证基础性能，专业专家提供专业化能力")
