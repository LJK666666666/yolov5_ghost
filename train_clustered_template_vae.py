...
from vae.clustered_template_vae import ClusteredTemplateManager, TEMPLATE_SIZE
...
class ClusteredTemplateDataset(Dataset):
    ...
    def _collect_samples(self, max_samples):
        ...
        if self.precomputed_labels_count > 0:
            efficiency = self.precomputed_labels_count / (self.precomputed_labels_count + self.computed_labels_count) * 100
    ...
    def _extract_sample(self, image, annotations, img_filename=None):
        ...
        if img_filename and sample:
            ...
            if cluster_labels:
                # 将图像级别的聚类标签转换为样本级别
                sample_cluster_labels = self._convert_image_labels_to_sample_labels(
                    cluster_labels, sample
                )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            loss, loss_components = compute_clustered_template_loss(
                ...
                true_clusters=true_clusters, beta=beta, gamma=gamma
            )
        ...
        for category, components in epoch_components.items():
            ...
            print(f"  {category}: Recon={avg_components['recon']:.4f}, "
