...
def analyze_model_structure(yaml_path, model_name):
    ...
    return {"se_count": se_count, "se_positions": se_positions, "total_layers": len(backbone) + len(head)}
...
def compare_models():
    ...
    models = {"YOLOv5s (原始)": "models/yolov5s.yaml", "YOLOv5s-SE (SE注意力)": "models/yolov5s-se.yaml"}
    ...
    if len(results) >= 2:
        ...
        if original and se_model:
            ...
            print("  • 计算量略微增加，推理速度可能稍微下降")
            print("  • 特征表示能力增强，可能提升检测精度")
            print("  • 对复杂场景和小目标检测可能有改善")
...
def generate_usage_guide():
    ...
    print("    --cfg models/yolov5s-se.yaml \\")
    ...
    print("\n🔧 推理命令:")
    print("python detect.py --weights runs/train/exp/weights/best.pt \\")
    print("    --source your_images \\")
    print("    --img 640")

    print("\n⚙️  参数调优建议:")
    print("• SE reduction ratio: 可以在SEBlock中调整降维比例")
    print("  - 较小值(8): 更强的注意力，但参数更多")
    print("  - 较大值(32): 更轻量，但注意力效果可能减弱")
    print("• 批次大小: 由于参数增加，可能需要适当减小batch size")
    print("• 学习率: 建议使用与原始YOLOv5s相同的学习率策略")

    print("\n📊 性能评估:")
    print("• 使用相同数据集对比原始YOLOv5s和YOLOv5s-SE")
    print("• 关注mAP@0.5和mAP@0.5:0.95指标")
    print("• 测试推理速度和内存使用情况")
    print("• 特别关注小目标和复杂场景的检测效果")
