...
class ParameterCalculator:
    ...
    def calculate_moe_layer_params(
        self, c1: int, c2: int, num_experts: int, top_k: int, k: int = 3, expert_type: str = "conv"
    ) -> Tuple[int, int]:
        """计算MoE层的总参数和激活参数."""
        # 专家网络参数
    ...
    def analyze_moe_model(self, config: Dict[str, Any], model_type: str) -> Tuple[int, int]:
        ...
        for layer_config in backbone:
            ...
            if module == "Conv":
                ...
                moe_t, moe_a = self.calc.calculate_moe_layer_params(current_channels, c2, num_experts, top_k, k)
                total_params += moe_t
...
def analyze_activation_details(yaml_path: str, model_type: str) -> Dict[str, Any]:
    """分析模型的激活参数详情."""
    details = {"moe_layers": [], "activation_strategy": "", "expert_configs": []}
            ...
            if details["moe_layers"]:
                ...
                for moe_layer in details["moe_layers"][:3]:  # 只显示前3个
                    if "top_k_range" in moe_layer:
                        print(
                            f"   └─ {moe_layer['module']}: {moe_layer['num_experts']}专家, "
