...
data_transforms = Compose(
    [Resize((IMG_HEIGHT, IMG_WIDTH)), ToTensor(), Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])]
)
...
class Encoder(nn.Module):
    def __init__(self, latent_dim, num_classes):
        ...
        self.conv_layers = nn.Sequential(
            # 示例结构, 实际需要根据图像大小设计
            nn.Conv2d(
                CHANNELS + 1, 32, kernel_size=4, stride=2, padding=1
            ),  # 输入通道为 CHANNELS+1，因为要拼接标签信息
            nn.ReLU(),
