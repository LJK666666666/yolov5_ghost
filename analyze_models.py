    ...
    try:
        ...
        with torch.no_grad():
            ...
            if isinstance(result, tuple) and len(result) >= 2:
                flops, _params = result[0], result[1]
            else:
                print("thop返回格式异常")
                return 0
...
def analyze_model(model_path, device, input_size=(640, 640), num_runs=100):
    ...
    try:
        ...
        print(
            f"✓ {model_info['model_name']}: {total_params:,} 参数, {gflops:.1f} GFLOPs, {inference_time:.2f}ms, {fps:.1f} FPS"
        )
...
def create_summary_report(models_info, output_path):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("-" * 120 + "\n")
        f.write(
            f"{'模型名称':<25} {'文件大小(MB)':<12} {'参数量':<15} {'GFLOPs':<8} {'推理时间(ms)':<12} {'FPS':<8} {'状态':<20}\n"
        )
        f.write("-" * 120 + "\n")
        ...
        for info in models_info:
            if info["error"] is None:
                ...
                f.write(
                    f"{info['model_name']:<25} {info['file_size_mb']:<12.2f} {info['total_params']:<15,} {gflops_str:<8} {info['inference_time_ms']:<12.2f} {info['fps']:<8.1f} {'成功':<20}\n"
                )
            else:
                error_msg = f"失败: {info['error']}"
                f.write(
                    f"{info['model_name']:<25} {info['file_size_mb']:<12.2f} {'N/A':<15} {'N/A':<8} {'N/A':<12} {'N/A':<8} {error_msg:<20}\n"
                )
        ...
        if successful_models:
            ...
            f.write(
                f"参数量最少模型: {smallest_params_model['model_name']} ({smallest_params_model['total_params']:,} 参数)\n"
            )
            ...
            if THOP_AVAILABLE:
                ...
                f.write(
                    f"计算量最少模型: {smallest_gflops_model['model_name']} ({smallest_gflops_model['gflops']:.1f} GFLOPs)\n"
                )
...
使用示例:
  ...
  python analyze_models.py -f runs/train200epoch -o analysis_output --device cpu
    ...
    parser.add_argument("--output", "-o", type=str, default=None, help="输出结果的文件夹路径 (默认: 与输入文件夹相同)")

    parser.add_argument("--device", type=str, default="", help="使用的设备 (默认: 自动选择)")

    parser.add_argument("--num-runs", type=int, default=100, help="推理速度测试的运行次数 (默认: 100)")
