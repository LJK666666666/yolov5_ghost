...
from clustered_template_vae import ClusteredTemplateManager, TEMPLATE_SIZE
...
def create_clustered_dataset(source_dir, output_dir):
    ...
    for split in ['train', 'val']:
        ...
        for img_file in tqdm(image_files, desc=f"处理{split}"):
            ...
            # 提取各类别样本并分配聚类
            sample_data = extract_and_cluster_sample(
                image, annotations, cluster_models, templates
            )
...
def save_cluster_analysis(output_dir, cluster_stats, cluster_samples, templates):
    ...
    stats_data = {
        ...
        'cluster_info': {
            category: {
                'num_clusters': len(templates[category]),
