...
def wiou_loss(box1, box2, xywh=True, eps=1e-7):
    """
    Calculates WIoU (Wise IoU) loss between two boxes, with dynamic weighting factor.

    WIoU reduces penalty on low-quality samples to improve model generalization.
    Reference: https://arxiv.org/abs/2301.10051

    Args:
        box1, box2: predicted and target boxes
        xywh: if True, boxes are in (x, y, w, h) format; if False, in (x1, y1, x2, y2) format
    ...
    # Aspect ratio term
    v = (4 / (3.14159**2)) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
    with torch.no_grad():
        alpha = v / (v - iou + (1 + eps))

    # WIoU loss calculation
    # R_WIoU: weighting factor that reduces penalty on low-quality samples
    r_wiou = torch.exp(-(rho2 / c2))

    # WIoU = 1 - IoU + (rho2/c2) * r_wiou + v * alpha * r_wiou
    wiou = 1 - iou + (rho2 / c2) * r_wiou + v * alpha * r_wiou
