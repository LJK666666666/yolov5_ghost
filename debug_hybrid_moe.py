...
def test_individual_classes():
    ...
    try:
        from models.common import SharedExpert, Expert, SparseGating, HybridMoELayer, HybridMoEBottleneck, C3HybridMoE
        print("✅ 所有类导入成功")
    ...
    # 测试SharedExpert
    try:
        shared_expert = SharedExpert(c1=64, c2=32)
    ...
    # 测试SparseGating
    try:
        gate = SparseGating(c1=64, num_experts=4, top_k=2)
    ...
    # 测试HybridMoELayer
    try:
        moe_layer = HybridMoELayer(c1=64, c2=64, num_experts=4, top_k=2, shared_ratio=0.25)
    ...
    try:
        moe_bottleneck = HybridMoEBottleneck(c1=64, c2=64, num_experts=4, top_k=2, shared_ratio=0.25)
    ...
    # 测试C3HybridMoE
    try:
        c3_hybrid_moe = C3HybridMoE(c1=64, c2=64, n=1, num_experts=4, top_k=2, shared_ratio=0.25)
...
def test_yaml_parsing():
    ...
    try:
        ...
        simple_config = {
            'backbone': [
                [-1, 1, 'Conv', [64, 6, 2, 2]],
                [-1, 1, 'C3HybridMoE', [64, True, 1, 0.5, 4, 2, 0.25]]
            ],
            'head': [
